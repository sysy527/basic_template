...
def main():
    ...
    if mode == "train":
        ...
        transform_train = transforms.Compose([Normalization(mean=0.5, std=0.5),
                                    RandomFlip(), ToTensor(), RandomCrop()])
        ### ver3
        ''''
        transform_train = transforms.Compose([Normalization(mean=0.5, std=0.5),
                                            RandomFlip(), ToTensor(), ToOnehot(), RandomCrop()])
        '''
        ...
        ## ver3. 
        '''
        transform_test = transforms.Compose([Normalization(mean=0.5, std=0.5), ToTensor(), ToOnehot()])
        '''
